...
export function Simulator() {
  ...
  const terms = useMemo(
    () => termOptions.map((value) => ({ value, label: `${value} ${value === 1 ? 'mes' : 'meses'}` })),
    [termOptions],
  );
  ...
  return (
    ...
    >
      ...
      <div className="pt-2 space-y-2.5">
        <FieldError
          message={sim.valid ? '' : sim.message}
          reserveSpace={false}
          iconSize={15}
          className={cn('text-sm transition-all', sim.valid ? 'h-0 overflow-hidden' : 'h-auto mb-2')}
          role={sim.valid ? undefined : 'alert'}
          aria-live="polite"
          aria-hidden={sim.valid ? true : undefined}
        />
        <ApplyButton origin="simulator" variant="default" size="block" disabled={!sim.valid} className="w-full min-h-[52px] h-[52px] bg-green text-ink hover:bg-green-bright disabled:opacity-40 shadow-sm hover:shadow-md transition-[transform,opacity,background-color,box-shadow] active:scale-[0.96] text-base font-bold border-0">
          Pedir mi crédito
        </ApplyButton>
      </div>
    </form>
